...
Deux surfaces de mémoire. Dans la nav (`consoleNav.ts`), seule **« données »** reste : elle vit dans l'**espace de travail**, à côté des projets — le groupe « mémoire » ne contenait plus qu'elle et a été aplati (oto#193) :
- **Datastore** (`/data`, `DataView.vue`) — stockage tabulaire, **substrat PG natif** (plus Google Sheets). Grille **server-driven** (`DataTable.vue` : tri 3 états/recherche/pagination/**filtres par colonne** côté API via `getNamespaceRows({offset,limit,order_by,order_dir,q,filters})` — ops par type dérivé `datastoreFilters.ts` (text/number/date/bool/**timestamp**), cellule `ColumnFilterCell.vue`, chips des filtres actifs retirables (`FilterChips.vue`, partagé avec la vue fiches), taille de page 25/50/100, header sticky ; rendu cellules typé `cellRender.ts`) ; clic row → détail/édition (`RowDrawer.vue`).
  ...
  > **« Enregistrer comme vue par défaut » retiré (12/09, oto#192)** : aucun usage en 45 jours. Le bouton du menu « colonnes » (`DataTable.vue`), `onSaveView`, `hiddenPatch` et `patchNamespaceSchema` sont partis ; le choix de colonnes reste un rendu local porté par `?cols=`, et le témoin « la couche API ne porte aucun `PUT …/schema` » reste dans `datastoreColumns.spec.ts`. Le paragraphe suivant raconte le défaut qu'il avait fermé.
  ...
  > **L'étape d'une fiche se dit comme telle (25/09, oto#140).** La colonne d'avancement (`role: "status"` + `lifecycle`) s'affichait en code brut : badge `A_QUALIFIER` à côté du titre, boutons « → non_interesse ◼ » sans rien qui dise de quoi il s'agissait. Désormais un bloc sous l'en-tête du drawer : « **Statut** (libellé de la colonne) **A qualifier** · Passer à : [A contacter] [Non interesse ◼ étape finale] » ; chaque bouton dit son geste en infobulle, une étape finale l'explique (« la fiche n'en sort plus »). Le libellé vient d'**une seule** règle, `etatLisible` (`lib/datastoreLifecycle.ts`) : `_` → espace, première lettre en capitale, **aucun accent ni traduction devinés** — **et depuis oto#140 (même jour), il lit d'abord `lifecycle.labels[état]`** : le libellé que le cycle de vie déclare (« À qualifier »), la règle ne servant plus qu'aux états sans libellé. `etatLisible(état, champ)` exige la colonne (ou `{ lifecycle }`) : un appelant qui l'oublierait afficherait le code dérivé à côté d'un écran qui affiche le libellé déclaré. Même libellé partout : barre de statuts (qui nomme aussi la colonne et marque les étapes finales), badge des fiches, cellule de liste, filtre de colonne (liste des étapes), chips de filtre, files de travail du projet, toast de transition ; le code brut reste en infobulle.
...
- **Deux gestes** (contrat d'écriture d'une case, oto#140, 23/09/2026) : **vider écrit
  ...
  une case non touchée n'est pas nommée. L'option « Laisser vide (rien trouvé) »
  (`VideAssumeToggle.vue`) écrit `@empty` (« cherché, rien »), se désactive quand on saisit
  une valeur, et annulée sans valeur écrit `null` (qui retire le marqueur). C'est une option
  **du champ**, posée sous sa saisie et offerte seulement quand elle est vide (ou déjà
  laissée vide) : la bascule « vide assumé » d'avant, flottant à côté du libellé même sur
  un champ rempli, se lisait comme un badge d'état (oto#140). Active, le champ dit
  « Laissé vide volontairement », suivi du `comment` de la case s'il existe. Offerte sur
...
- **Changement** (`components/console/ActivityChange.vue`, `valueChangesOf`) : avec
  ...
  60 caractères (entier en infobulle), `@empty` = « laissé vide », clé absente = « (absent) ».
