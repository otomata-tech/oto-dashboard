...
- **Un rôle ne vaut pas droit d'écrire** : un geste d'écriture lit `canAdministerOrg` ou `canWriteInOrg` (`useMe`), jamais le rôle seul — en consultation (`active_org_readonly`) le serveur refuse tout, super_admin compris ; la coque le dit une fois (`ConsultOrgBanner`), l'écran omet ses gestes (oto#211), `/connectors` compris (oto#212). « Voir en tant que » n'est pas couvert : aucun champ servi ne le dit, et la règle ne le déduit pas. → `docs/conventions.md`, `docs/orgs-groupes-invitations.md`
