...
> **Direction artistique « 2a » (contraste chaud).** Sidebar **encre** ancrant un contenu crème, cartes qui flottent sur un **filet doux + ombre chaude** (jamais de bord noir), rayons **8px ou pill uniquement**, boutons **tous pill + casse normale**, typo Familjen Grotesk + **Spline Sans Mono**, icônes **Lucide**, logo **« O ouvert »**. Le *pourquoi* : `oto-studio/brand/charte-doc/DESIGN-BRIEF.md`, §0-3 (le dossier `design-system/` qui le portait a été archivé le 2026-08-27 — `docs/design-system.md`). Skill front : `.claude/skills/oto-frontend`. **Ce fichier est le contrat des écrans servis** ; l'identité, elle, se définit dans `oto-studio/brand/`.
...
- **Table client-side** : composant **`ConsoleTable`** — LE composant des tables admin/monitoring (liste chargée en un fetch). Porte le chrome `.tbl`, les états chargement/vide et la **pagination intégrée** (`usePager`, 25/page, `Pager` en pied). `#head` = les `<th>`, `#row="{ row }"` = la/les `<tr>` (drill-down multi-`<tr>` OK). Monter dans une carte `flush`. Ne PAS recopier `table.tbl` + états + `Pager` à la main dans une vue. **Recherche et tri intégrés** (24/09/2026) : `:search="(r) => texte"` ajoute le champ (sans casse ni accents), `:sorts="{ clé: (r) => valeur }"` + `:default-sort` rendent des colonnes triables, dont la tête s'écrit avec **`SortTh`** (`#head="{ sort }"` puis `<SortTh k="clé" :sort="sort">`). Plus de champ de filtre maison posé à côté du tableau. Pendant **server-driven** : `DataTable` (tri/filtres côté API).
- **Lien vers une fiche plateforme** : composant **`EntityRef`** (`kind` user|org, `id`, `label`) — tout nom de compte ou d'org affiché sur un écran `/platform/*` mène à sa fiche, de la même façon ; `@click.stop` intégré pour les lignes elles-mêmes cliquables.
